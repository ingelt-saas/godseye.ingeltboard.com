...
const EditModal = ({ open, close, item, refetch }) => {
    ...
    const updateHandler = async (e) => {
        ...
        if (!selectedFile && subject === item.subject && name === item.name) {
            toast.error('No Changes Here');
            return;
        }
        ...
        try {
            await libraryApi.update(item.id, formData);
    ...
    return (
        <Modal open={open} onClose={handleClose} className="grid place-items-center">
            <div className="bg-white rounded-xl p-4 sm:w-[500px]">
                <h2 className="pb-2 border-b text-xl font-medium">Update {item?.name} library item</h2>
                <div className="">
